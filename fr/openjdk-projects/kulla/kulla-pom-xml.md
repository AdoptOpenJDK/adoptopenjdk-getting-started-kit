# pom.xml Maven pour construire votre propre kulla.jar

Enregistrez le ```xml``` ci-dessous en ```pom.xml```, et placez le dans le répertoire ```$HOME/sources/kulla-dev/langtools/repl/``` :
...
Puis lancez la commande Maven suivante :

```
$ mvn clean package -Dbuild.image.folder=$JAVA9_HOME
```

où ```$JAVA9_HOME``` pointe vers le binaire du JDK 9 téléchargé depuis le [site JDK9 Early Access](https://jdk9.java.net/download/).

ou ce qui suit si vous avez construit votre propre image du JDK à partir de la forêt ```kulla-dev``` (le repértoire build doit avoir les images de créées):
